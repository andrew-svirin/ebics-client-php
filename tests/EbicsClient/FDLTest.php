 ...
 */
class FDLTest extends TestCase
{
    public function testOkWithXml(): void
    {
        ...
        $requestHandler->expects(self::once())->method('buildFDL')->with($bank, $user, $keyRing, $date, 'myFile', 'FR', null, null)->willReturn($request);
        ...
        self::assertEquals($responseXml, $sUT->FDL($bank, $user, $keyRing, 'myFile', 'xml', 'FR', $date));
    }

    public function testWithPlain(): void
    {
        $requestMaker     = self::createMock(RequestMaker::class);
        $requestHandler   = self::createMock(RequestHandler::class);
        $responseHandler  = self::createMock(ResponseHandler::class);
        ...
        $responseXml->expects(self::once())->method('addTransaction')->with($transaction);
        $responseHandler->expects(self::once())->method('retrieveOrderData')->with($responseXml)->willReturn($orderDataEncrypted);
        $cryptService->expects(self::once())->method('decryptOrderDataContent')->with($keyRing, $orderDataEncrypted)->willReturn('test');
        $transaction->expects(self::once())->method('setPlainOrderData')->with('test');
        ...
        self::assertEquals($responseXml, $sUT->FDL($bank, $user, $keyRing, 'myFile', 'plain', 'FR', $date));
    }
}
